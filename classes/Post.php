...
class Post {
    ...
    public function submitPost($body, $user_to, $imageName){
        ...
        $body = mysqli_real_escape_string($this->connect, $body); //Removes single quotes if user type npr I'am
        $check_empty = preg_replace('/\s+/', '', $body);//Deletes all spaces

        if($check_empty != ""){


            //Put youtube on the page over URL
            ...
            $query = mysqli_query($this->connect, "INSERT INTO posts VALUES('','{$body}','{$added_by}','{$user_to}','{$date_added}','no','no','0','{$imageName}')");
            $returned_id = mysqli_insert_id($this->connect);

            //Insert notification
            if($user_to != 'none'){
                $notification = new Notification($this->connect, $added_by);
...
$user_details_query = mysqli_query($this->connect, "SELECT firstName, lastName, profile_pic FROM users WHERE username='$added_by'");
    ...
    $str .= "<div class='status_post' onClick='javascript:toggle$id()'>
      ...
      if($count > $limit)
      $str .= "<input type='hiden' class='nextPage' value='" . ($page + 1) . "'>
...
public function loadProfilePosts($data, $limit){
    ...
    $str = "";//String to return
    $data_query = mysqli_query($this->connect, "SELECT * FROM posts WHERE deleted='no' AND ((added_by='$profileUser' AND user_to='none') OR user_to='$profileUser') ORDER BY id DESC");
...
public function getSinglePost($post_id){
        ...
        $str = "";//String to return
        $data_query = mysqli_query($this->connect, "SELECT * FROM posts WHERE deleted='no' AND id='$post_id'");
